...
namespace hsx {
	auto hsx_data_from_label(HspLabel const* ptr) -> HspData {
		return HspData{ HspType::Label, (PDAT const*)ptr };
	}

	auto hsx_data_from_str(char const* ptr) -> HspData {
		return HspData{ HspType::Str, (PDAT const*)ptr };
	}

	auto hsx_data_from_double(HspDouble const* ptr) -> HspData {
		return HspData{ HspType::Double, (PDAT const*)ptr };
	}

	auto hsx_data_from_int(HspInt const* ptr) -> HspData {
		return HspData{ HspType::Int, (PDAT const*)ptr };
	}

	auto hsx_data_from_flex(FlexValue const* flex) -> HspData {
		return HspData{ HspType::Struct, (PDAT const*)flex };
	}

	auto hsx_data_to_label(HspData const& data)->std::optional<HspLabel> {
	...
	auto hsx_data_to_str(HspData const& data)->std::optional<char const*> {
	...
	auto hsx_data_to_double(HspData const& data)->std::optional<HspDouble> {
	...
	auto hsx_data_to_int(HspData const& data)->std::optional<HspInt> {
	...
	auto hsx_data_to_flex(HspData const& data)->std::optional<FlexValue const*> {
